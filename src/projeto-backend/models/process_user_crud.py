...
def get_process_user_by_user_id_all(db: Session, user_id: int, skip: int = 0, limit: int = 100):
    ...
    return db.query(ProcessUser).filter(ProcessUser.user_id==user_id).offset(skip).limit(limit).all()

def get_process_user_by_process_id_all(db: Session, process_id: int, skip: int = 0, limit: int = 100):
    """Busca todas as relação do processo com usuarios e retorna os 100 primeiros
     registro com capacidade para ir seguindo a busca"""
    return db.query(ProcessUser).filter(ProcessUser.process_id==process_id).offset(skip).limit(limit).all()
